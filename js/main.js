 ...
 * Main JavaScript File v3.0 - Modern Liquid Glass Edition
...
const Shop = {
    ...
    handleOrder(productType, card) {
        ...
        const messages = {
            'size-s': 'Здравствуйте! Хочу заказать журнал размера S (до 12 страниц) за 3800₽. Расскажите, пожалуйста, подробнее о процессе заказа.',
            'size-m': 'Здравствуйте! Меня интересует журнал размера M (до 16 страниц) за 4800₽. Как можно оформить заказ?',
            'size-l': 'Привет! Хотел бы заказать журнал размера L (до 20 страниц) за 5800₽. Что нужно для начала работы?',
            'size-xl': 'Добрый день! Интересует журнал размера XL (до 30 страниц) за 7800₽. Можете рассказать о возможностях?',
            'travel-book': 'Здравствуйте! Хочу заказать Travel Book на 20 страниц за 3800₽. Какие материалы нужно подготовить?',
            'poster': 'Здравствуйте! Хочу заказать постер за 2000₽. Рамка и печать входят в стоимость?',
            'express': 'Добрый день! Мне нужен срочный заказ EXPRESS за 9800₽. Какие сроки и условия?',
            'template': 'Привет! Интересует готовый вариант журнала за 2800₽. Можно посмотреть доступные шаблоны?'
        };
        
        const managerMessage = messages[productType] || `Здравствуйте! Хочу заказать ${title} за ${price}. Расскажите подробнее об услуге.`;
...
const App = {
    init() {
        console.log('🎨 A&A Design Web App v3.0 - Liquid Glass Edition');
